...
export function calculateTrend(symbol: string, candles: Candle[]): TrendResult {
  ...
  // Read the latest candle as the current market state.
  const latest = candles[candles.length - 1];

  // Read the previous candle for short-term price direction.
  ...
  // Normalize ATR to price so different coins share the same scale.
  const atrPercent = latest.close === 0 ? 0 : (latestAtr / latest.close) * 100;

  // Compare the latest volume with the previous 20 candles.
  ...
  // Keep bullish and bearish evidence separate so the result stays explainable.
  let bullishPoints = 0;
  let bearishPoints = 0;
  ...
  // Activity contributes to score but cannot dominate directional evidence.
  ...
  // Reward useful activity without allowing volume to create direction by itself.
